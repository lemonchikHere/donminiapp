...
def create_test_properties(db_session: Session):
    prop1 = Property(
        id=uuid4(), telegram_message_id=1, telegram_channel_id=123, posted_at=datetime(2024, 1, 1),
        transaction_type='sell', property_type='apartment', rooms=2, price_usd=100000, is_active=True,
        address="Центр города, ул. Пушкина", photos='["photo1.jpg"]'
    )
    prop2 = Property(
        id=uuid4(), telegram_message_id=2, telegram_channel_id=123, posted_at=datetime(2024, 1, 2),
        transaction_type='rent', property_type='house', rooms=4, price_usd=2000, is_active=True,
        address="Тихий район, ул. Есенина", photos='["photo2.jpg"]'
    )
    prop3 = Property(
        id=uuid4(), telegram_message_id=3, telegram_channel_id=123, posted_at=datetime(2024, 1, 3),
        transaction_type='sell', property_type='apartment', rooms=3, price_usd=150000, is_active=True,
        address="Новостройка на набережной", photos='["photo3.jpg"]'
    )
...
def test_search_with_budget_filter(client: TestClient, db_session: Session):
    ...
    create_test_properties(db_session)
    response = client.post("/api/search", json={"budget_min": 90000, "budget_max": 110000})
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
...
def test_search_semantic_query(client: TestClient, db_session: Session, mock_openai_for_semantic_search):
    ...
    fake_property = Property(
        id=uuid4(), telegram_message_id=99, telegram_channel_id=123, posted_at=datetime.now(),
        transaction_type='sell', property_type='apartment', rooms=2, price_usd=123456, is_active=True,
        address="Mocked Address", photos='[]'
    )
