...
from .search import PropertyResponse, SearchResponse  # Reuse response models
...
async def get_property_details(
    ...
    return PropertyResponse(
        ...
        telegram_link=(
            f"https://t.me/c/{prop.telegram_channel_id}/" f"{prop.telegram_message_id}"
        ),
...
async def get_similar_properties(
    ...
    if not source_prop or not source_prop.embedding:
        raise HTTPException(
            status_code=404,
            detail="Source property or its embedding not found",
        )
    ...
    return SearchResponse(results=properties_response, total=len(properties_response))
